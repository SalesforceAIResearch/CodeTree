...
from common import wrap_mbpp_data, wrap_human_eval_data, gen_test_eval


ADD_HINT = "To solve the problem, you can refer the hint given by an expert: "

# TODO: ADD feature to combine nodes
class Node:
...
def rerank_list_of_nodes(list_of_nodes):
    return sorted(list_of_nodes, key=lambda x:x.value) # small value in the front
...
def run_dfs(
    ...
    for idx, item in enumerate(dataset):
        ...
        if is_passing:
            ...
            print_v(f'completed {idx + 1}/{num_items}: acc = {round(num_success / (idx + 1), 3)}, weak_acc={item["weak_acc"]}, pass no solve: {sad_case}, enter debug: {debug_case}, "exhaust": {debug_thoroughed_case}', "skip", skip)
